...
void CacheFile::Sort()
{
   ...
   SortCache = *this;
   std::qsort(List.get(), getCache()->Head().PackageCount, sizeof(List[0]), NameComp);
}
									/*}}}*/
...
bool ShowList(std::ostream &out, const std::string &Title, std::string List, const std::string &VersionsList, size_t l_ScreenWidth)
{
   ...
   // Normally, packages in List are separated by a space; but when we
   // are called from ShowEssential, List has additional information,
