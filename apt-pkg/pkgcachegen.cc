...
pkgCacheGenerator::pkgCacheGenerator(DynamicMMap &aMap,OpProgress *Prog) :
		    Map(aMap), Cache(aMap,false), Progress(Prog),
		    FoundFileDeps(0)
{
   CurrentFile = 0;
				  ...
				  pkgCache::VerIterator *OutVer)
{
   ...
   while (List.Step() == true)
   {
	 ...
	 for (; Ver.end() == false; Last = &Ver->NextVer, Ver++)
	 {
      ...
      // Add a new version
      if (auto const verindex = NewVersion(Ver,Version,*Last))
         *Last = *verindex;
      else
				   ...
				   ListParser &List)
{
   ...
   // First, fill in a structure. If ok, then allocate and link.
   pkgCache::VerFile NewVerFile;
...
std::optional<unsigned long> pkgCacheGenerator::NewVersion(pkgCache::VerIterator &Ver,
					    const string &VerStr,
					    unsigned long Next)
{
   // Get a structure
...
std::optional<unsigned long> pkgCacheGenerator::WriteUniqString(const char *S,
						 unsigned int Size)
{
...
std::unique_ptr<MMap> pkgMakeStatusCache(pkgSourceList &List,OpProgress &Progress,
                                         bool AllowMem)
{
   const unsigned long MapSize = getConfiguredCacheLimit(__func__);
